...
from lang.ru.bot import (
    ...
    HOWTO_QUE1, HOWTO_QUE3,
    HOWTO_YES, HOWTO_NO,
    HOWTO_EDU1, HOWTO_EDU2, HOWTO_EDU3, HOWTO_EDU4, HOWTO_EDU5,
    HOWTO_RESULT, HOWTO_FAILED, STAGES_BACK,
    HOWTO_LEARNING_JOB, HOWTO_LEARNING_JOB_ANSWER
)
from keyboards.howto import simple_answer, simple_answer_que, education, howto
...
def register_howto_handlers(dp: Dispatcher):
    ...
    async def handle_start(message: Message):
        await message.answer(START_, reply_markup=howto, parse_mode="MarkdownV2")

    @router.message(F.text == START_HOWTO)
    async def handle_que1(message: Message, state: FSMContext):
        await message.answer(HOWTO_QUE1, reply_markup=simple_answer, parse_mode="MarkdownV2")
    ...
    async def handle_que2(message: Message, state: FSMContext):
        await message.answer(HOWTO_QUE3, reply_markup=simple_answer_que, parse_mode="MarkdownV2")
        await state.set_state(HowToSteps.step2)

    @router.message(HowToSteps.step2, F.text == HOWTO_LEARNING_JOB)
    async def handle_que2(message: Message, state: FSMContext):
        await message.answer(HOWTO_LEARNING_JOB_ANSWER, reply_markup=simple_answer, parse_mode="MarkdownV2")

    @router.message(HowToSteps.step2, F.text == HOWTO_YES)
    async def handle_final(message: Message, state: FSMContext):
        await message.answer(HOWTO_RESULT, reply_markup=howto, parse_mode="MarkdownV2")
        await state.clear()
    ...
    async def handle_failed(message: Message, state: FSMContext):
        await message.answer(HOWTO_FAILED, reply_markup=howto, parse_mode="MarkdownV2")
        await state.clear()
